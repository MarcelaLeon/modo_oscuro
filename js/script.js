...
/*---------------Entrada tipo confirm ----------------------*/ 
/*let confirmacion = confirm("¿Desea quitar modo oscuro?")
if (confirmacion){
    body.classList.remove("dark");
}*/


/*-----------------Seleccion tipo button-------------------*/
...
/*----------------Boton deslizable-input tipo checkbox-----------------------*/
...
let item = document.querySelector(".contenedor")

for (let dato in datos){
    item.innerHTML+=`<div class="item">
    <img src="${datos[dato].imagen}">
    <h2>${datos[dato].titulo}<h2>
    <p>${datos[dato].parrafo}<p>
    </div>`
}